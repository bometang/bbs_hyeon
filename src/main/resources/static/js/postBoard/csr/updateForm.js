...
// ---------- 등록 ----------
async function addPostBoard(data) {                                // :contentReference[oaicite:9]{index=9}
  const { header } = await ajax.post('/api/postBoards', data);
  if (header.rtcd === 'S00') window.location.href = '/boards';
...
function displayForm() {                                           // :contentReference[oaicite:10]{index=10}
  ...
  wrap.innerHTML = `
    <form id="frm">
      ...
      <label>작성자 <input      name="nickname" readonly></label><br>
