...
public class PostCommentDAOImpl implements PostCommentDAO{
  ...
  //수동매핑
  private RowMapper<PostComment> doRowMapper(){

    return (rs, rowNum)->{
      PostComment postComment = new PostComment();
      postComment.setCommentId(rs.getLong("comment_id"));
      postComment.setPostId(rs.getLong("post_id"));
      postComment.setParentId(rs.getLong("post_id"));
      postComment.setDepth(rs.getLong("depth"));
      postComment.setContent(rs.getString("content"));
      postComment.setMemberId(rs.getLong("member_id"));
      postComment.setCodeId(rs.getString("code_id"));
      postComment.setCdate(rs.getTimestamp("cdate").toLocalDateTime());
      return postComment;
    };
  }
  ...
  public Long save(PostComment postComment) {
    ...
    sql.append("VALUES (postBoard_post_id_seq.nextval,:postId,:parentId,:depth,:content, :memberId) ");
  ...
  public List<PostComment> findAll(Long id, int pageNo, int numOfRows) {
    ...
    List<PostComment> list = template.query(sql.toString(), params, doRowMapper());
  ...
  public int getTotalCount(Long id) {
    String sql = "select count(*) from postBoard where post_id= :id";
